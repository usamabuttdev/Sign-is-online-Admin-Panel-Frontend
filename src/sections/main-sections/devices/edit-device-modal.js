import { yupResolver } from "@hookform/resolvers/yup";
import PropTypes from "prop-types";
import { useEffect } from "react";
import { useForm } from "react-hook-form";
import * as Yup from "yup";
import LoadingButton from "@mui/lab/LoadingButton";
import Box from "@mui/material/Box";
import Button from "@mui/material/Button";
import Dialog from "@mui/material/Dialog";
import DialogActions from "@mui/material/DialogActions";
import DialogContent from "@mui/material/DialogContent";
import DialogTitle from "@mui/material/DialogTitle";
import MenuItem from "@mui/material/MenuItem";
import FormProvider, { RHFTextField, RHFSelect } from "src/components/hook-form";
import { useSnackbar } from "src/components/snackbar";
import { useUpdateDeviceMutation } from "src/store/Reducer/devices";

export default function EditDeviceForm({ row, open, onClose }) {
  const { enqueueSnackbar } = useSnackbar();

  const [updateDevice, { isLoading: isUpdating }] = useUpdateDeviceMutation();

  const EditDeviceSchema = Yup.object().shape({
    location_id: Yup.string(),
    hardware_type: Yup.string(),
    firmware_version: Yup.string(),
    status: Yup.string(),
    last_heartbeat: Yup.string(),
  });

  const defaultValues = {
    location_id: "",
    hardware_type: "",
    firmware_version: "",
    status: "active",
    last_heartbeat: "",
  };

  const methods = useForm({
    resolver: yupResolver(EditDeviceSchema),
    defaultValues,
  });

  const {
    reset,
    handleSubmit,
    formState: { isSubmitting, dirtyFields },
  } = methods;

  useEffect(() => {
    if (row) {
      reset({
        location_id: row?.location_id || "",
        hardware_type: row?.hardware_type || "",
        firmware_version: row?.firmware_version || "",
        status: row?.status || "active",
        last_heartbeat: row?.last_heartbeat || "",
      });
    }
  }, [row, reset]);

  const onSubmit = handleSubmit(async (data) => {
    if (Object.keys(dirtyFields).length === 0) {
      enqueueSnackbar("No changes detected!", { variant: "warning", autoHideDuration: 2000 });
      return;
    }

    try {
      await updateDevice({ id: row.id, data }).unwrap();
      enqueueSnackbar("Device updated successfully!", { variant: "success" });
      reset();
      onClose();
    } catch (error) {
      console.error("Unexpected Error:", error);
      enqueueSnackbar(error?.data?.message || 'An error occurred', { variant: 'error' });
    }
  });

  return (
    <Dialog
      fullWidth
      maxWidth={false}
      open={open}
      onClose={onClose}
      PaperProps={{
        sx: { maxWidth: 550 },
      }}
    >
      <FormProvider methods={methods} onSubmit={onSubmit}>
        <DialogTitle>Edit Device</DialogTitle>

        <DialogContent>
          <Box
            mt={1}
            rowGap={3}
            columnGap={2}
            display="grid"
            gridTemplateColumns={{
              xs: "repeat(1, 1fr)",
              sm: "repeat(1, 1fr)",
            }}
          >
            <RHFTextField name="location_id" label="Location ID" />
            <RHFTextField name="hardware_type" label="Hardware Type" />
            <RHFTextField name="firmware_version" label="Firmware Version" />
            <RHFSelect name="status" label="Status">
              <MenuItem value="active">Active</MenuItem>
              <MenuItem value="inactive">Inactive</MenuItem>
            </RHFSelect>
            <RHFTextField name="last_heartbeat" label="Last Heartbeat" />
          </Box>
        </DialogContent>

        <DialogActions>
          <Button variant="outlined" onClick={onClose}>
            Close
          </Button>

          <LoadingButton
            type="submit"
            sx={{ px: 3 }}
            color="primary"
            variant="contained"
            loading={isUpdating}
          >
            Update
          </LoadingButton>
        </DialogActions>
      </FormProvider>
    </Dialog>
  );
}

EditDeviceForm.propTypes = {
  row: PropTypes.object,
  onClose: PropTypes.func,
  open: PropTypes.bool,
};
...
import { useDeleteLanguageMutation } from 'src/store/Reducer/languages';
...
export default function UserTableRow({ row, selected, counter }) {
  const { flag, title, createdAt, isActive } = row;
  ...
  const handleDelete = async () => {
    const response = await deleteLanguage(row?._id);
    if (!response.error) {
    ...
    <>
      <TableRow hover selected={selected}>
        ...
        <TableCell sx={{ display: 'flex', alignItems: 'center' }}>
          <Avatar alt={title} src={flag} sx={{ mr: 2 }} />
        </TableCell>

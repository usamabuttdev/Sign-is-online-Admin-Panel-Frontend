...
import { useDeleteTrainingModeMutation } from 'src/store/Reducer/training-modes';
...
export default function UserTableRow({ row, selected, counter }) {
  ...
  const [deleteTrainingMode, { isLoading }] = useDeleteTrainingModeMutation();

  const handleDelete = async () => {  
    try {
      const response = await deleteTrainingMode(row?._id);
      if (!response.error) {
